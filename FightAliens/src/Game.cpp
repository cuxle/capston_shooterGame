	...
	std::size_t target_frame_duration) 
{
	...
	//init player
	m_player = new FighterPlane(100, 100, 0, m_PlayerTexture, &m_bullets);
...
void Game::spawnEnemies()
{
	if (--AlienPlane::m_SpawnTimer <= 0) {
		...
		AlienPlane *enemy = new AlienPlane(SCREEN_WIDTH, rand() % SCREEN_HEIGHT, -(2 + (rand() % 4)), m_EnemyTexture);
...
bool Game::bulletHitFighter(BattleObject *bullet)
{
	for (auto fighter : m_fighters) {
